...
export class LFUCache<K, V> implements Cache<K, V> {
  ...
  set(key: K, value: V): void {
    ...
    if (this.values.size >= this.capacity) {
      let minFreq = Infinity, minKey: K | undefined;
      for (const [k, f] of this.freq) {
        if (f < minFreq) { minFreq = f; minKey = k; }
      }
      if (minKey !== undefined) { this.values.delete(minKey); this.freq.delete(minKey); }
    }
    this.values.set(key, value);
    this.freq.set(key, 1);
  }
  ...
  delete(key: K): boolean { this.freq.delete(key); return this.values.delete(key); }
